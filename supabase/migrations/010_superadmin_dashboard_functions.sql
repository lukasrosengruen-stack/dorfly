-- Super-Admin Dashboard Helper Functions
-- SECURITY DEFINER erlaubt Zugriff auf auth.users für MAU/Retention-Berechnung

-- ─── Bürger-Statistiken ──────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION superadmin_buerger_stats(p_gemeinde_id uuid DEFAULT NULL)
RETURNS json
LANGUAGE sql
SECURITY DEFINER
...
AS $$
  SELECT json_build_object(
    'total',               COUNT(*),
    'mau',                 COUNT(*) FILTER (WHERE u.last_sign_in_at > NOW() - INTERVAL '30 days'),
    'retention_eligible',  COUNT(*) FILTER (WHERE u.created_at < NOW() - INTERVAL '30 days'),
    'retention_active',    COUNT(*) FILTER (
...
-- ─── Mängel-Statistiken ──────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION superadmin_maengel_stats(p_gemeinde_id uuid DEFAULT NULL)
RETURNS json
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT json_build_object(
    'total',          COUNT(*),
    'erledigt',       COUNT(*) FILTER (WHERE status::text = 'erledigt'),
    'in_bearbeitung', COUNT(*) FILTER (WHERE status::text = 'in_bearbeitung'),
    'offen',          COUNT(*) FILTER (WHERE status::text = 'offen')
  )
  FROM public.maengel
  WHERE (p_gemeinde_id IS NULL OR gemeinde_id = p_gemeinde_id);
$$;
...
-- ─── Berechtigungen: nur service_role darf diese Funktionen aufrufen ─────────
REVOKE EXECUTE ON FUNCTION superadmin_buerger_stats(uuid)                  FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION superadmin_rollen_stats(uuid)                   FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION superadmin_maengel_stats(uuid)                  FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION superadmin_posts_stats(uuid)                    FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION superadmin_produzentenaccounts(text, uuid)      FROM PUBLIC;

GRANT EXECUTE ON FUNCTION superadmin_buerger_stats(uuid)              TO service_role;
GRANT EXECUTE ON FUNCTION superadmin_rollen_stats(uuid)               TO service_role;
GRANT EXECUTE ON FUNCTION superadmin_maengel_stats(uuid)              TO service_role;
GRANT EXECUTE ON FUNCTION superadmin_posts_stats(uuid)                TO service_role;
GRANT EXECUTE ON FUNCTION superadmin_produzentenaccounts(text, uuid)  TO service_role;
