...
type EinladungRolle = 'buerger' | 'verein' | 'organisation' | 'gewerbe'
...
interface Einladung {
  ...
  erstellt_am: string
  ablauft_am: string
  eingeladen_von_profil?: { display_name: string | null } | null
}
...
function neueZeile(): EinladungZeile {
  return { id: String(++zeilenCounter), email: '', rolle: 'buerger', orgName: '', hinweis: '' }
}

export default function EinladungenSection() {
  ...
  const [neueRolle, setNeueRolle] = useState<EinladungRolle>('buerger')
  ...
  async function nutzerSuchen() {
    if (!suchEmail.trim()) return
    setSuchLaden(true)
    setGefundenerNutzer(null)
    try {
      const res = await fetch(`/api/verwaltung/nutzer/suche?email=${encodeURIComponent(suchEmail.trim())}`)
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? `HTTP ${res.status}`)
  ...
  async function rolleSpeichern() {
    if (!gefundenerNutzer || gefundenerNutzer === 'nicht-gefunden') return
  ...
  return (
    <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <h2 className="text-lg font-semibold text-gray-900 mb-4">Nutzer & Rollen</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              disabled={sending}
              className="flex items-center gap-2 bg-primary-500 hover:bg-primary-600 disabled:opacity-50 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors"
            >
        ...
        <div className="space-y-5">
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={suchLaden || !suchEmail.trim()}
          ...
          {gefundenerNutzer === 'nicht-gefunden' && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
              Kein Nutzer mit dieser E-Mail in Ihrer Gemeinde gefunden.{' '}
              <button
          ...
          {gefundenerNutzer && gefundenerNutzer !== 'nicht-gefunden' && (
            <div className="border border-gray-200 rounded-xl p-4 space-y-4">
              ...
              {neueRolle !== gefundenerNutzer.role && neueRolle === 'buerger' && gefundenerNutzer.role !== 'buerger' && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
                  Die aktuelle Rolle wird entzogen. Der Nutzer wird per E-Mail informiert.
                </div>
              )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-600 disabled:opacity-50 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors"
              >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-gray-600">Eingeladen</th>
