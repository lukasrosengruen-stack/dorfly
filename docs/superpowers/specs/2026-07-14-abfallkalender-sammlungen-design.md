...
**Korrektur nach Code-Recherche:** [VereinPostForm.tsx](../../../src/features/verein/VereinPostForm.tsx) ist nicht in Benutzung (wird von keiner Seite importiert). Die tatsächlich aktive Komponente, über die eingeloggte Vereine/Organisationen ihre Beiträge erstellen und bearbeiten, ist [VereinPostVerwaltung.tsx](../../../src/components/dashboard/VereinPostVerwaltung.tsx) (gerendert von `(admin)/dashboard/page.tsx:106-118` für die Rollen `verein`/`organisation`), mit eigenem `TAGS`-Array (Zeile 44) und `submitNew`/`submitEdit`, die beide `/api/verein/post` aufrufen. Diese Komponente wird erweitert, nicht `VereinPostForm.tsx`.

Kategorie „Sammlung“ wird zusätzlich freigeschaltet (nur für Vereine — [GewerbePostForm.tsx](../../../src/features/gewerbe/GewerbePostForm.tsx) bleibt unverändert bei Nachricht/Bekanntmachung). Gleiche drei Zusatzfelder; **Organisation/Verein** wird mit dem eigenen `vereine.verein_name` vorbefüllt, bleibt aber editierbar (z. B. falls eine Untergruppe wie „Jugendfeuerwehr Musterdorf“ sammelt). Zod-Schema für `POST /api/verein/post` ([src/lib/validations.ts:266-288](../../../src/lib/validations.ts#L266-L288)) wird um die drei Felder ergänzt (`sammlung_art` als `z.enum(...)`, `sammlung_datum` als Datums-String, `sammlung_organisator` als nicht-leerer String), jeweils `.optional()` und nur bei `tag === 'sammlung'` per `.refine(...)` verpflichtend.
